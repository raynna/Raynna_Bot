...
class LastMonth {
    ...
    async execute(tags, channel, argument, client, isBotModerator) {
        try {
            ...
            const { id } = player;
            const { data: xpGains, errorMessage: xpMessage } = await getData(RequestType.XPGains, id, experiencePeriods.LAST_MONTH);
            if (xpMessage) {
            ...
            return `${player.name} gained in total of ${xp} experience & ${level} levels last month.`;
        } catch (error) {
