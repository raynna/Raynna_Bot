...
class Year {
    ...
    async execute(tags, channel, argument, client, isBotModerator) {
        try {
            ...
            const { id } = player;
            const { data: xpGains, errorMessage: xpMessage } = await getData(RequestType.XPGains, id, experiencePeriods.YEAR);
            if (xpMessage) {
            ...
            return `${player.name} has gained in total of ${xp} experience & ${level} levels this year.`;
        } catch (error) {
