...
class LastWeek {
    ...
    async execute(tags, channel, argument, client, isBotModerator) {
        try {
            ...
            const { id } = player;
            const { data: xpGains, errorMessage: xpMessage } = await getData(RequestType.XPGains, id, experiencePeriods.LAST_WEEK);
            if (xpMessage) {
            ...
            return `${player.name} has gained in total of ${xp} experience & ${level} levels last week.`;
        } catch (error) {
