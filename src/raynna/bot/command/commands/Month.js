...
class Month {
    ...
    async execute(tags, channel, argument, client, isBotModerator) {
        try {
            ...
            const { id } = player;
            const { data: xpGains, errorMessage: xpMessage } = await getData(RequestType.XPGains, id, experiencePeriods.MONTH);
            if (xpMessage) {
            ...
            return `${player.name} gained in total of ${xp} experience & ${level} levels this month.`;
        } catch (error) {
